...
public class HeadTracking_Player : MonoBehaviour
{
    ...
    public float TrackingRadius = 10.0f;
    ...
    private void Tracking()
    {
        ...
        if (tracking != null && targets.Length > 0 && !Player.IsStop && !Player.ComboStateData.IsCombo)
        {
        ...
        BodyRig.weight = Mathf.Lerp(BodyRig.weight, 0.0f, Time.deltaTime * WeightSpeed);
    }

    private void Tracking_Aiming()
    {
        AimTargetTransform.DOMove(transform.position + Camera.main.transform.forward * 50.0f, RetargetSpeed);
        HeadRig.weight = Mathf.Lerp(HeadRig.weight, 1.0f, Time.deltaTime * WeightSpeed);
        BodyRig.weight = Mathf.Lerp(BodyRig.weight, 1.0f, Time.deltaTime * WeightSpeed);
    }
