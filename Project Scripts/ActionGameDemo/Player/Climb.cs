...
public class Climb : MonoBehaviour
{
    ...
    {
        if ((InputSystemManager.instance.PlayerController.Locomotion.Climb.triggered || !GetOwner.IsGrounded) && IsCheckClimbing && !IsClimbing)
        {
    ...
    private void SetClimbUI(bool isEnable, Vector3 point = default)
    {
        ClimbUI.SetActive(isEnable);
        if (isEnable) ClimbUI.transform.position = Camera.main.WorldToScreenPoint(point);
    }
}
